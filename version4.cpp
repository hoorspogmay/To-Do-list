...
//consits all of the functions used in main
class TaskManager {
	private:
		Task* head;
		int taskCount;
...
//notifyuser
		...
		void clearFile() {
			...
			if (!outFile) {
				cout << "\nError opening file for clearing!" << endl;
				return;
			}
			outFile.close();
			cout << "\nFile cleared successfully." << endl;
		}
...
// Hardcoded tasks
void initializeTasks(TaskManager& taskManager) {
	taskManager.addTask("Task 1", "Work", "30");
	taskManager.addTask("Task 2", "Personal", "60");
	taskManager.addTask("Task 3", "Study", "120");
	taskManager.addTask("Task 4", "Work", "45");
	taskManager.addTask("Task 5", "Other", "90");
}
// Main function
int main() {
